...
export class Init1640399209022 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`user_mail_address\` (\`user\` int NOT NULL AUTO_INCREMENT, \`mailAddress\` varchar(255) NOT NULL, PRIMARY KEY (\`user\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`user\` (\`userId\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, PRIMARY KEY (\`userId\`)) ENGINE=InnoDB`);
    }
